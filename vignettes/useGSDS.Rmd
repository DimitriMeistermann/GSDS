...
Here the gene modules (gene coexpression clusters) are computed with a Leiden clustering, by default, from a correlation distance matrix.
The activation scores are then computed for each module for a quick visualization.

A good practice is to rename module names by an important leading gene in each module. 
Here we rename the modules using the JASPAR transcription factor (TF) database.
The best TF of each module in term of PC1 contribution will be used to rename the module 

```{r geneModules, fig.dim = c(10,7)}
...
```{r htDetailedModules, fig.dim = c(11,11)}
